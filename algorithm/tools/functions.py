...
def loader_anim(msg="Analyzing..."):
    spinner = ['|', '/', '-', '\\']

    i = 0
    while not global_loading:
        sys.stdout.write(f"\r{msg} {spinner[i % len(spinner)]}")
        sys.stdout.flush()
        i += 1
        time.sleep(0.1)
    sys.stdout.write("\r[OK] Done analyzing")
...
def g_adjust_HW(h, w):
    ...
    if max_dim < 300:
        ...
        block_size, step = 16, 6
    elif max_dim < 1080:
        block_size, step = 16, 8
    elif max_dim < 1440:
        block_size, step = 16, 32
    else:  
        block_size, step = 16, 62

    print(f"[AUTO] Image {w}x{h} -> block_size={block_size}, step={step}")

    return block_size, step
