...
class Spatial_Domain:
    ...
    def noise(self, local_mean: int, varience: int) -> None:
        ...
        features = extractNoise(norm_var, prnu_map, noise_estimation)

        return features

    
    def copyMove(self, blockSize: int, steps: int, threshold: int) -> None:

        norm_gry = g_grayScale(self.img_input)  
        h, w = norm_gry.shape

        block_size, step = g_adjust_HW(h, w)
        blocks = []
        position = []
    ...
    def resampling(self) -> None:
        ...
        features = extractResampling(spectrum)
